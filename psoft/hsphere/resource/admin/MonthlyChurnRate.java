...
/* loaded from: hsphere.zip:psoft/hsphere/resource/admin/MonthlyChurnRate.class */
public class MonthlyChurnRate implements TemplateHashModel {
    protected Date begin;
    protected Date end;

    public MonthlyChurnRate(int year, int month) {
    ...
    protected TemplateModel getInfo() throws Exception {
        ...
        try {
            ...
            TreeSet sortedGroup = new TreeSet(new Comparator() { // from class: psoft.hsphere.resource.admin.MonthlyChurnRate.1
    ...
    protected List getPlanList(long groupId) throws Exception {
        ...
        PreparedStatement ps = null;
        try {
            ps = con.prepareStatement("SELECT g.plan_id, p.description  FROM cmp_plan_group g, plans p WHERE p.deleted = 0 AND g.group_id = ? AND  g.plan_id = p.id AND p.reseller_id = ?  ORDER BY p.description");
    ...
    protected List getUngroupedPlanList() throws Exception {
        ...
        PreparedStatement ps = null;
        try {
            ps = con.prepareStatement("SELECT id, description  FROM plans  WHERE deleted = 0 AND reseller_id = ? AND id NOT IN (  SELECT plan_id FROM cmp_plan_group) ORDER BY description");
            ps.setLong(1, Session.getResellerId());
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                plans.add(new TemplateHash(getInfoForPlan(rs.getString(1), rs.getString(2))));
    ...
    protected Hashtable getInfoForPlan(String planId, String description) throws Exception {
        ...
        long beginAmount = 0;
        long endAmount = 0;
        long accountsAmount = 0;
        Connection con = Session.getDb();
        PreparedStatement ps = null;
        try {
            PreparedStatement ps2 = con.prepareStatement(" SELECT count(*) FROM accounts  WHERE  plan_id = ? AND created < ? AND  ( deleted IS NULL OR deleted >= ?)");
            ps2.setString(1, planId);
            ps2.setDate(2, this.begin);
            ps2.setDate(3, this.begin);
            ResultSet rs = ps2.executeQuery();
            if (rs.next()) {
                plan.put("begin_amount", rs.getString(1));
                beginAmount = rs.getLong(1);
            }
            ps2.setString(1, planId);
            ps2.setDate(2, this.end);
            ps2.setDate(3, this.end);
            ResultSet rs2 = ps2.executeQuery();
            if (rs2.next()) {
                plan.put("end_amount", rs2.getString(1));
                endAmount = rs2.getLong(1);
            }
            ps2.close();
            ps = con.prepareStatement(" SELECT count(*) FROM accounts  WHERE plan_id = ?   AND created >= ? AND  created < ? AND (failed <> 1 OR failed IS NULL)");
            ps.setString(1, planId);
            ps.setDate(2, this.begin);
            ps.setDate(3, this.end);
            ResultSet rs3 = ps.executeQuery();
            if (rs3.next()) {
                plan.put("accounts", rs3.getString(1));
                accountsAmount = rs3.getLong(1);
            }
            long goOutAmount = (beginAmount + accountsAmount) - endAmount;
            plan.put("go_out", Long.toString(goOutAmount));
            Session.closeStatement(ps);
            con.close();
            return plan;
        } catch (Throwable th) {
    ...
    public TemplateModel get(String key) throws TemplateModelException {
        try {
            ...
            Session.getLog().error("Error geting Signup/Go-away rate.", ex);
            return null;
        }
    }
