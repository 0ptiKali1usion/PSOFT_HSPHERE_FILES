...
/* loaded from: hsphere.zip:psoft/hsphere/resource/admin/lservers/MailServerInfo.class */
public class MailServerInfo extends LogicalServerInfo {
    ...
    public List getInfo(long lserverId) throws Exception {
        ...
        try {
            ps = con.prepareStatement("SELECT count(*) FROM mail_services WHERE mail_server = ?");
            ps.setLong(1, lserverId);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
                Hashtable infos = new Hashtable();
                infos.put("name", "mail_domains");
