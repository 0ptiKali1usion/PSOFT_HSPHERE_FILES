...
/* loaded from: hsphere.zip:psoft/hsphere/resource/admin/lservers/PGSQLServerInfo.class */
public class PGSQLServerInfo extends LogicalServerInfo {
    ...
    public List getInfo(long lserverId) throws Exception {
        ...
        try {
            PreparedStatement ps2 = con.prepareStatement("SELECT count(*) FROM pgsqlres WHERE pgsql_host_id = ?");
            ps2.setLong(1, lserverId);
            ResultSet rs = ps2.executeQuery();
            if (rs.next()) {
                Hashtable infos = new Hashtable();
                infos.put("name", "pgsql_res");
                infos.put("value", Integer.toString(rs.getInt(1)));
                list.add(infos);
            }
            ps = con.prepareStatement("select count(*) from parent_child p, pgsqlres s where p.parent_id = s.id and s.pgsql_host_id = ?");
    ...
    public String getUsed(long lserverId) throws Exception {
        ...
        PreparedStatement ps = null;
        try {
            ps = con.prepareStatement("SELECT count(*) FROM pgsqlres WHERE pgsql_host_id = ?");
            ps.setLong(1, lserverId);
