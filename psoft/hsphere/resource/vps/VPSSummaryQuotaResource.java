...
/* loaded from: hsphere.zip:psoft/hsphere/resource/vps/VPSSummaryQuotaResource.class */
public class VPSSummaryQuotaResource extends Quota {
    ...
    public VPSSummaryQuotaResource(ResourceId id) throws Exception {
        super(id);
        this.usageLoadTime = 0L;
        this.lastDayUsageLoadTime = 0L;
    }

    public VPSSummaryQuotaResource(int type, Collection initValues) throws Exception {
    ...
    public BillEntry usageCharge(Date end) throws Exception {
        ...
        String descr = getUsageChargeDescription(beginDate, endDate);
        ...
        if (Session.getResellerId() != 1) {
            resDescr = getResellerUsageChargeDescription(beginDate, endDate);
            if (resAmount > 0.0d) {
    ...
    public double getUsageMultiplier() {
        try {
            Session.getLog().debug("In VPSSummaryQuota own getUsageMultiplier");
            ...
            Session.getLog().debug("VPSSummaryQuota getUsageMultiplier = " + usedDisk + " = " + paidUsage);
    ...
    public TemplateModel get(String key) throws TemplateModelException {
        return ("limit".equals(key) || "limitMb".equals(key)) ? new TemplateString(this.size) : ("used".equals(key) || "usedMb".equals(key)) ? new TemplateString(smartLabel(getUsage())) : "lastDayUsedMb".equals(key) ? new TemplateString(smartLabel(getLastDayUsage(true))) : "info".equals(key) ? new TemplateString(info()) : super.get(key);
    }
    ...
    public boolean suspendLimit() {
        ...
        return (getUsage() * 100.0d) / ((double) (this.size * 1024)) > ((double) susp);
    }
    ...
    public String info() {
        try {
            return Localizer.translateMessage("quota.overlimitinfo", new String[]{"VPS Summary quota", "account", String.valueOf(Session.getAccount().getId().getId()), smartLabel(getUsage()), Integer.toString(this.size)});
        } catch (Exception e) {
            return "";
        }
    }
