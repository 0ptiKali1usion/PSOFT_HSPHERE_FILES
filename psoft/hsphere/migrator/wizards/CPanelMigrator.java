...
/* loaded from: hsphere.zip:psoft/hsphere/migrator/wizards/CPanelMigrator.class */
public class CPanelMigrator extends Migrator {
    ...
    protected void setupJob(String line) throws MalformedURLException, UnknownResellerException, Exception {
        ...
        CPanelMigrationJob job = new CPanelMigrationJob(url, map, login, password, plan, bpid, balance, startDate);
    ...
    public TemplateModel FM_start() throws Exception {
        ...
        URL url = new URL("ftp://" + request.getParameter("ftplogin") + ":" + URLEncoder.encode(request.getParameter("ftppassword"), LanguageManager.STANDARD_CHARSET) + "@" + request.getParameter("ftpserver") + "/" + request.getParameter("ftpfile"));
        CPanelMigrationJob job = new CPanelMigrationJob(url, map, request.getParameter("login"), request.getParameter("password"), Plan.getPlan(request.getParameter("plan_id")).getDescription(), request.getParameter("bpselect"), request.getParameter("balance"), request.getParameter("start_date"));
        MigrationCron.addJob(job);
        return job;
    }
}
