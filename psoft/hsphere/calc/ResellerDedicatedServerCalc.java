...
/* loaded from: hsphere.zip:psoft/hsphere/calc/ResellerDedicatedServerCalc.class */
public class ResellerDedicatedServerCalc {
    public static double calc(Resource _r, Date begin, Date end) throws Exception {
        ...
        if (ds.isTemplatedServer()) {
            ...
            amount = Session.getReseller().getPrices(prefix).getRecurrentPrice();
        }
        Session.getLog().debug("ResellerDedicatedServerCalc::calc(Resource, Date, Date) result is " + (amount * r.getSetupMultiplier()) + " amount is " + amount);
    ...
    public static double calc(InitToken token) throws Exception {
        ...
        Session.getLog().debug("ResellerDedicatedServerCalc::calc(InitToken) result is " + (amount * token.getSetupMultiplier()) + " amount is " + amount);
